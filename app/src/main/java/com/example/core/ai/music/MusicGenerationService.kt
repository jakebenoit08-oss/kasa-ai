...
class KasaMusicGenerationService(
  ...
  private fun classifyHttpError(statusCode: Int, errorBody: String?): AppError {
    ...
    return when (statusCode) {
      ...
      401 -> AppError.Unauthorized(
        "Music service authentication error (HTTP 401). Please verify the AIMUSIC_API_KEY in the backend configuration."
      )
      403 -> {
        if (body.contains("CREDIT_LIMIT_REACHED", ignoreCase = true)) {
          AppError.ServiceUnavailable("You have used your music generation allowance for this subscription period.")
        } else if (body.contains("INSUFFICIENT_CREDITS", ignoreCase = true)) {
          AppError.ServiceUnavailable("AIMusicAPI account has insufficient provider credits (HTTP 403).")
        } else {
    ...
    try {
      val response = try {
        ...
        if (baseUrl == AppConfig.MUSIC_BACKEND_EMULATOR_URL) {
          try {
            createApiService(AppConfig.MUSIC_BACKEND_LOCALHOST_URL).getCredits(userId)
          } catch (e2: Exception) {
  ...
  override suspend fun generateMusic(
    ...
    onStatusUpdate: (String) -> Unit
  ): AppResult<GeneratedSong> = withContext(Dispatchers.IO) {
    ...
    val createResponse = try {
      val res = try {
        apiService.createMusic(createRequest)
      } catch (e: IOException) {
        if (baseUrl == AppConfig.MUSIC_BACKEND_EMULATOR_URL) {
          try {
            createApiService(AppConfig.MUSIC_BACKEND_LOCALHOST_URL).createMusic(createRequest)
          } catch (e2: Exception) {
            return@withContext AppResult.Error(classifyNetworkError(e2, AppConfig.MUSIC_BACKEND_LOCALHOST_URL))
          }
        } else {
    ...
    // 2. Poll the task endpoint every 15-20 seconds as documented
    ...
    while (pollCount < maxPollAttempts) {
      ...
      val pollResponse = try {
        val res = try {
          apiService.getTaskStatus(taskId, userId)
        } catch (e: IOException) {
          if (baseUrl == AppConfig.MUSIC_BACKEND_EMULATOR_URL) {
            createApiService(AppConfig.MUSIC_BACKEND_LOCALHOST_URL).getTaskStatus(taskId, userId)
          } else {
            throw e
          }
        }
      ...
      if (status == "succeeded") {
        val clip = taskData.clip
        val audioUrl = clip?.audioUrl

        if (!audioUrl.isNullOrBlank()) {
          onStatusUpdate("Song ready 🎵")
          val resolvedTitle = clip.title?.takeIf { it.isNotBlank() }
          ...
          musicRepository.saveSong(generatedSong)
          return@withContext AppResult.Success(generatedSong)
        } else {
          return@withContext AppResult.Error(
            AppError.AiEngineError("Song completed but no audio URL was returned by provider.")
          )
        }
